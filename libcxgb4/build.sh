...
if [[ ! -a ./rdma-core ]] ; then
	...
	if [[ "x$rdma_core_version" != "x" ]] ; then
		...
		# echo "System installed with pre-rdma-core packages or no rdma-core ..."
		commit=v60.0
		IBV_INST=""
	fi
	...
	if [[ "x${RDMA_CORE}" != "x" ]] ; then
		...
		git clone ${RDMA_CORE} rdma-core >/dev/null 2>&1
	else
		echo "Cloning rdma-core from github..."
		git clone --quiet https://github.com/linux-rdma/rdma-core || { echo "Clone failed: $?" ; }
	fi

	(
	cd rdma-core
	echo "Checking out rdma-core commit $commit..."
	if [[ "x$dist_ubuntu" != "x" ]] ; then
		git checkout -f debian/${rdma_core_version%ubuntu*} >/dev/null 2>&1
	else
		git checkout -f $commit >/dev/null 2>&1
	fi

	if [[ $commit = "v14" || $commit = "v13" ]] ; then
		patch -sN -p1 < ../patches/fix_static_build.patch
	fi
	)
